...
abstract contract CitiesSignatureClaim is EIP712, ICitiesSignatureClaim {
    ...
    bytes32 internal constant TYPEHASH =
        keccak256(
            "ClaimRequest(address to,address[] inTokenAddresses,uint256 outTokenId,uint128 validityStartTimestamp,uint128 validityEndTimestamp,bytes32 uid)"
        );
    ...
    function _processRequest(StakeRequest calldata _req, bytes calldata _signature) internal returns (address signer) {
        bool success;
    ...
    function _encodeRequest(StakeRequest calldata _req) internal pure returns (bytes memory) {
        return
            abi.encode(
                TYPEHASH,
                _req.to,
                keccak256(abi.encodePacked(_req.inTokenAddresses)),  
                _req.outTokenId,
